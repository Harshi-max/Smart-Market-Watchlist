...
import CompetitorComparisonModal from "@/components/competitor-comparison-modal";
import { getCompetitorProfile } from "@/lib/competitor-data";
...
function HomeContent() {
  ...
  const openSmartPilotWithQuestion = (q: string) => {
    setPilotQuestion(q);
    setPilotOpen(true);
  };
  ...
  const competitorProfile = useMemo(() => getCompetitorProfile(selected.symbol), [selected.symbol]);
  ...
  return (
    <>
      ...
      <section className="main-content">
        ...
        <div className="content-wrap" id="top">
          ...
          <div className="dashboard-grid">
            <section className="panel trace-panel"><div className="panel-heading"><div><span className="eyebrow">WHY IT MATTERS</span><h2>Insight Trace <span className="live-pill"><span className="status-dot" /> Live</span></h2></div><button className="icon-button subtle"><MoreHorizontal size={18} /></button></div><div className="trace-stock"><div className={`ticker-logo ${selected.tone}`}>{selected.symbol.slice(0, 2)}</div><div><strong>{selected.symbol}</strong><span>{selected.name} · {selected.sector}</span></div><div className="trace-price"><strong className={selected.tone}>{selected.change}</strong><span>₹{selected.price}</span></div></div><div className="trace-flow"><TraceItem label="Price movement" value={selected.change} note="Compared to previous close" tone={selected.tone} /><TraceItem label="Market-relative" value={`${competitorProfile.marketChangePct >= 0 ? "+" : ""}${competitorProfile.marketChangePct.toFixed(1)}%`} note={`${competitorProfile.marketIndex} · ${competitorProfile.marketChangePct >= 0 ? "+" : ""}${competitorProfile.marketChangePct.toFixed(1)}%`} tone={competitorProfile.marketChangePct >= 0 ? "positive" : "negative"} /><TraceItem label="Sector-relative" value={`${competitorProfile.sectorChangePct >= 0 ? "+" : ""}${competitorProfile.sectorChangePct.toFixed(1)}%`} note={`${selected.sector} sector`} tone={competitorProfile.sectorChangePct >= 0 ? "positive" : "negative"} /><TraceItem label="Volume anomaly" value={selected.volume} note="vs. 20-day average" tone="warning" /></div><button className="expand-trace" onClick={() => setTraceOpen(!traceOpen)}>{traceOpen ? "Hide evidence details" : "Expand evidence details"}<ChevronRight size={15} className={traceOpen ? "rotate-90" : ""} /></button>{traceOpen && <div className="evidence-detail"><div><ShieldCheck size={16} /><span><strong>Data confidence 92%</strong> · preferred source confirmed</span></div><p>{selected.reason} This is a relative movement estimate, not a causal conclusion.</p></div>}<div className="significance-result"><div className="result-score">{selected.score}</div><div><span className="eyebrow">SIGNIFICANCE</span><strong>{selected.score > 75 ? "High" : "Moderate"}</strong><p>{selected.reason}</p></div></div></section>

            <section className="panel isolation-panel" id="isolation"><div className="panel-heading"><div><span className="eyebrow">SIGNAL ISOLATION</span><h2>Company-specific signal</h2></div><CircleHelp size={16} className="muted-icon" /></div><p className="panel-intro">Would this movement still look meaningful if the market and sector had not moved?</p><div className="isolation-score"><div className="ring"><span>{competitorProfile.isolationScore}</span><small>/ 100</small></div><div><strong>{competitorProfile.isolationLevel} isolation</strong><span>Relative movement estimate</span></div></div><div className="compare-bars"><CompareBar label={selected.symbol} value={competitorProfile.isolationScore} color="teal" /><CompareBar label={`Sector (${selected.sector})`} value={Math.round(Math.min(Math.abs(competitorProfile.sectorChangePct) * 18 + 25, 90))} color="slate" /><CompareBar label={`Market (${competitorProfile.marketIndex})`} value={Math.round(Math.min(Math.abs(competitorProfile.marketChangePct) * 18 + 18, 90))} color="slate" /></div><div className="isolation-callout"><Zap size={15} /><span>{competitorProfile.synthesis.detail}</span></div><button className="outline-button compare-competitors-action" onClick={() => setCompetitorModalOpen(true)}><Scale size={15} /> Compare with competitors ({competitorProfile.competitors.length} peers) <ChevronRight size={15} /></button></section>
          </div>
        ...
        <aside className="right-rail">
          <section className="rail-card pilot-rail"><div className="rail-heading"><div><span className="eyebrow">SMARTPILOT</span><h2>Hi Arjun!</h2></div><span className="beta-pill">BETA</span><button className="rail-close">×</button></div><div className="pilot-orb-large"><BrainCircuit size={28} /></div><p>How can I help you today?</p><div className="rail-prompts"><button onClick={() => openSmartPilotWithQuestion("What changed while I was away?")}>What changed while I was away?</button><button onClick={() => openSmartPilotWithQuestion("Which stock needs my attention?")}>Which stock needs my attention?</button><button onClick={() => openSmartPilotWithQuestion(`Why is ${selected.symbol} moving today?`)}>Why did {selected.symbol} move today?</button><button onClick={() => openSmartPilotWithQuestion(`Compare ${selected.symbol} with its peers`)}>Compare {selected.symbol} with its peers</button></div><button className="voice-button" onClick={() => setPilotOpen(true)}><Mic size={15} /> Tap to speak</button></section>
      ...
      {pilotOpen && <SmartPilotDrawer initialQuestion={pilotQuestion} onClose={() => { setPilotOpen(false); setPilotQuestion(""); }} />}
      {competitorModalOpen && (
        <CompetitorComparisonModal
          initialSymbol={selectedSymbol}
          onClose={() => setCompetitorModalOpen(false)}
          onSelectStock={(sym) => setSelectedSymbol(sym)}
          onAskSmartPilot={(q) => openSmartPilotWithQuestion(q)}
        />
      )}
